...
function deposit(id) {
    ...
    // Set Amount - Check if it's already set to prevent adding repeatedly
    const currentAmount = $("#deposit_amount").val();
    if (!currentAmount || currentAmount === "0" || parseFloat(currentAmount) !== parseFloat(amount)) {
...
$(".amount-tooltips .btn").click(function () {
    ...
    // Find the closest parent container to identify which payment method this is for
    const parentContainer = $(this).closest('[id]');
    if (parentContainer.length) {
        const paymentType = parentContainer.attr('id');
        // Only set the amount for this specific payment method
        $("#" + paymentType + " .amount").val(amount);

        // Update specific values based on payment type
...
$("[data-tab]").click(function () {
    ...
    // Set the amount only once, not for all fields
    $("#" + payment + " .amount").val(amount);

    // Only update the specific values needed
    if (payment === "bitcoin") {
        $("#bitcoin_amt_value").text(amount);
    } else if (payment === "upi") {
        $("#upi_amount_txt").text(amount);
...
$(document).ready(function() {
    ...
    // Only set these values if they're not already set or are empty
    // This prevents adding the amount repeatedly on refresh
    const bitcoin_amt = $("#bitcoin_amount").val();
    if (!bitcoin_amt || bitcoin_amt === "0") {
        // Don't update if there's already a value
        $("#bitcoin_amt_value").text("");
    } else {
        $("#bitcoin_amt_value").text(bitcoin_amt);
    }

    const upi_amt = $("#upi_amount").val();
    if (!upi_amt || upi_amt === "0") {
        // Don't update if there's already a value
        $("#upi_amount_txt").text("");
    } else {
