...
export default function AuthRegister() {
  ...
  const customHandleChange = async (data) => {
    setRefer_id(data)
    if (!data || data.length < 8 || data.length % 2 !== 0) return;
    checkRefID(data)
  };
      ...
      <Formik
        ...
        onSubmit={async (values, { setErrors, setStatus, setSubmitting }) => {
          try {
            ...
            await register(refer_id, values.firstname, values.lastname, values.email, values.password, values.phone_number)
              .then(() => {
          ...
          return (
            <form noValidate onSubmit={handleSubmit}>
              <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                  <Stack spacing={1}>
                    <InputLabel htmlFor="phone_number-signup">Phone Number*</InputLabel>
                    <OutlinedInput
                      fullWidth
                      error={Boolean(touched.phone_number && errors.phone_number)}
                      id="phone_number-login"
                      type="text"
                      value={values.phone_number}
                      name="phone_number"
                      onBlur={handleBlur}
                      onChange={handleChange}
                      placeholder="7001XXXXX4"
                      inputProps={{}}
                    />
                  </Stack>
